...
public class FireCar extends Car {
    ...
    public  void soundHorn() {
        if (isRunning()) {
            /* runninStatus 는 private 필드이므로 접근 불가하며
             * 부모의 isRunning() 메소드를 통해서만 주행 중인 상태를 확인할 수 있으므로 메소드의 접근 제한자를 protected로 변경한다.
             * Car 클래스의 runningStatus를 protected로 변경하여 직접 접근 할 수도 있지만 캡슐화의 원칙에 위배되므로 권장 하지 않는다.
             * this.isRunning() or super.isRunning() 둘 다 사용 가능하며 컴파일러는 기본적으로 this. 을 추가한다.
             * (상황에 따라서는 super.을 명시적으로 써야 하는 상황도 있다.)*/
